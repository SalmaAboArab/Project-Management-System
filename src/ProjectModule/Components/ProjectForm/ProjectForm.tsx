...
export default function ProjectForm() {
    ...
    const onSubmitAdd= async(data:any)=>{
        ...
        // console.log(data);
        try {
            let response=await axios.post(`${baseUrl}/Project`,data,{headers:{Authorization:token}});
    ...
    const onSubmitUpdate= async(data:any)=>{
        ...
        // console.log(data);
        try {
            let response=await axios.put(`${baseUrl}/Project/${curruntProjectId}`,{'title':data.title?data.title:currentProject.title,'description':data.description?data.description:currentProject.description},{headers:{Authorization:token}});
    ...
    const getCurrentProject=async()=>{
        try {
            let response=await axios.get(`${baseUrl}/Project/${curruntProjectId}`,{headers:{Authorization:token}});
  ...
  return (
    ...
    <div className="ProjectFormHeader bg-white ps-5 py-4">
       ...
       <form onSubmit={handleSubmit(action=='add'?onSubmitAdd:onSubmitUpdate)} className={`${styles.ProjectForm} rounded-4 bg-white shadow container  w-75`}>
       <div className={`mt-5 py-5 px-4 ProjectFormInputs`}>
        ...
        defaultValue={action=='add'?'':currentProject?.title}
        {...register('title',
        action=='add'?{required: "Project title is required"}:{})}/>
            </div>
        ...
        defaultValue={action=='add'?'':currentProject?.description}
        {...register('description',
        action=='add'?{required: "Project description is required"}:{}
        )}
        ></textarea>
        </div>
        ...
        <div className='line border-top py-4 px-4 row justify-content-between mt-3'>
            ...
            <button className='btn border border-black rounded-5 col-md-5' onClick={()=>navigate('/dashboard/project-list')}>Cancel</button>
            </div>
